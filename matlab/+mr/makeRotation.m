...
assert( ( abs(theta) >= 0 ) & ( abs(theta) < 2*pi) , 'makeRotation:invalidTheta',...
        'rotation angle theta (%2.f) is invalid. should be within [0,2*pi] radians',theta);
assert( ( abs(phi) >= 0 ) & ( abs(theta) < pi) , 'makeRotation:invalidPhi',...
        'rotation angle phi (%2.f) is invalid. should be within [0,pi] radians',phi);
